...
const fetchTrivia = async () => {
  try {
    ...
    for (
      ...
      const retrievedCategoryObject = trivia_categories.find(
        (categoryObject) => categoryObject.name === category
      );
      const { id, name, difficulty } = getDifficultyLevel(
        retrievedCategoryObject,
        categoryTrivia
      );
      ...
      for (
        ...
        try {
          ...
          console.log(
            `${getDate()}: Successfully saved ${name} of difficulty ${difficultyLevel} `
          );
        } catch (err) {
          console.log(
            `${getDate()}: Failed to save ${name} of difficulty ${difficultyLevel} ${err}`
          );
        }
      }
